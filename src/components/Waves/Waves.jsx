...
export default function Waves({
    ...
    const getContentHeight = () => {
        ...
        const adjustedHeight = Math.max(scrollHeight - TOP_OFFSET, window.innerHeight);
    ...
    useEffect(() => {
        ...
        function setLines() {
            const { width } = boundingRef.current;
            const height = lastHeightRef.current || getContentHeight();
            linesRef.current = [];
        ...
        function onTouchMove(e) {
            ...
            updateMouse(touch.pageX, touch.pageY);
        }

        function updateMouse(x, y) {
            const mouse = mouseRef.current, b = boundingRef.current;
            mouse.x = x - b.left;
            mouse.y = y - b.top; // b.top includes the 150px offset
            if (!mouse.set) {
                mouse.sx = mouse.x;
                mouse.sy = mouse.y;
                mouse.lx = mouse.x;
        ...
        const observer = new MutationObserver(() => {
            ...
            if (newHeight > lastHeightRef.current && newHeight < 100000) {
                onResizeOrChange();
            }
        });
        ...
        window.addEventListener("load", onResizeOrChange);
        window.addEventListener("mousemove", onMouseMove);
        window.addEventListener("touchmove", onTouchMove, { passive: false });

        return () => {
            window.removeEventListener("resize", onResizeOrChange);
            window.removeEventListener("load", onResizeOrChange);
    ...
    return (
        <div
            ...
            style={{
                position: "absolute",
                top: TOP_OFFSET, // 150px offset
                left: 0, margin: 0, padding: 0,
                width: "100%",
                height: `${getContentHeight()}px`, // Adjusted height
                overflow: "hidden",
                backgroundColor,
                ...style
            }}
